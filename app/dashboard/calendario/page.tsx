...
export default function CalendarioPage() {
  ...
  const [mostrarEventosDia, setMostrarEventosDia] = useState(false)
  ...
  const cambiarMes = (delta: number) => setMesActual(new Date(year, month + delta, 1))
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl md:text-2xl font-bold text-[#F4F6FB]">🗓️ Calendario</h2>
      ...
      {/* Eventos del día seleccionado — solo móvil */}
      {mostrarEventosDia && (
        <div className="md:hidden bg-[#131B2E] rounded-xl border border-[#1E293B] p-4">
          <div className="flex justify-between items-center mb-3">
            <h3 className="font-semibold text-[#F4F6FB] text-sm">{diaSeleccionado}</h3>
            <button onClick={() => setMostrarEventosDia(false)} className="text-[#8C97B5] text-xs">✕ Cerrar</button>
          </div>
                ...
                <div key={e.id} className={`p-2 rounded-lg border text-xs ${tiposInfo[e.tipo].color}`}>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => toggleCompletado(e.id, e.completado)} className="opacity-60 hover:opacity-100">{e.completado ? '↺' : '✓'}</button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Calendario */}
        <div className="md:col-span-2 bg-[#131B2E] rounded-xl border border-[#1E293B] p-4 md:p-6">
          <div className="flex items-center justify-between mb-4">
            <button onClick={() => cambiarMes(-1)} className="text-[#8C97B5] hover:text-[#F4F6FB] px-2 text-lg">←</button>
                ...
                >
                  ...
                  <div className="flex gap-0.5 mt-0.5 flex-wrap justify-center">
                    {eventosDelDia.slice(0, 2).map(e => (
                      <span key={e.id} className={`w-1 h-1 rounded-full ${tiposInfo[e.tipo].dot}`} />
                    ))}
                  </div>
                </button>
              )
              ...
              <div className="space-y-2">
                {proximosEventos.map(e => (
                  <div key={e.id} className="flex items-center gap-2 text-xs">
                    <span className={`w-2 h-2 rounded-full flex-shrink-0 ${tiposInfo[e.tipo].dot}`} />
                    <span className="text-[#F4F6FB] flex-1 truncate">{e.titulo}</span>
